 ...
 *
 * a simple socket server that display the file contents
 * of the host's current root directory on the server b-
 * -rowser interface.
 *
 * History
 *
 *  v1.2   zhuximin@ruijie.com.cn        2018-3-25
 *         1.Strict compliance with coding standards,
 *           the comments are all in English
 *         2.On the original basis, modified the ser-
 *           -ver interface some unnecessary display
 *           of the bug
 *
 *  v1.1   zhuximin@ruijie.com.cn        2017-10-27
 *         1.the exit() funtion is no longer used to
 *           reduce the possibility of resource lea-
 *           -kage
 *         2.delete useless variable
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
#define DEFAULTIP   "127.0.0.1"                  /* IP address of server */
#define DEFAULTPORT "80"                         /* port of server */
#define DEFAULTBACK "10"                         /* XXX :back  */
#define DEFAULTDIR  "/home"                      /* the path to display the contents
                                                    of the current host */
#define MAXBUF        1024                       /* the maximum length of the buffer */
#define MAXPATH       150                        /* the maximum length of a directory or file name */
#define MAXSIZE       10                         /* the maximum length of option struct */

struct option long_options[MAXSIZE];             /* XXX :wait perfect*/
